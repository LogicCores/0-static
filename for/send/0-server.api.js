...
//                    options.subUri &&
                    ...
                    return LIB.fs.existsAsync(sourcePath).then(function (exists) {
                        if (!exists) {
                            return tryNextPath();
                        }
                        ...
                        return LIB.fs.existsAsync(distPath).then(function (exists) {
                            ...
                            var relativeBasePath = LIB.path.dirname(uri);
                            ...
                            return LIB.fs.readFileAsync(sourcePath, "utf8").then(function (data) {
                				...
                				.use(REWORK_PLUGIN_URL(function (url) {
                				    ...
                				    if (
                				        options.stripCssPrefix &&
                				        new RegExp("^" + LIB.RegExp_Escape(options.stripCssPrefix + "/" + relativeBasePath), "g").exec(url)
                				    ) {
                				        url = url.replace(new RegExp("^" + LIB.RegExp_Escape(options.stripCssPrefix + "/" + relativeBasePath), "g"), ".");
                				    }
    /*
